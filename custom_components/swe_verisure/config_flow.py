"""Config flow for the Swe Verisure integration."""
...
class VerisureConfigFlowHandler(ConfigFlow, domain=DOMAIN):
        ...
        if user_input is not None:
            ...
            self.verisure = Verisure(
                ...
                cookie_file_name=self.hass.config.path(
                    STORAGE_DIR, f"swe_verisure_{user_input[CONF_EMAIL]}"
                ),
            )
    ...
    async def async_step_installation(
        ...
        for installation in (
            ...
            installation_id = installation.get("giid")
            if not installation_id:
                continue
            alias = installation.get("alias") or installation_id
            street = (installation.get("address") or {}).get("street")
            installations[installation_id] = (
                f"{alias} ({street})" if street else alias
            )
